...
def reg_l1_loss(pred, target, mask, channel=2):
    # --------------------------------#
    #   计算l1_loss
    # --------------------------------#
    pred = pred.permute(0, 2, 3, 1)
    expand_mask = torch.unsqueeze(mask, -1).repeat(1, 1, 1, channel)
...
class MscLovaszSoftmax(nn.Module):

    def __init__(self, ignore_index=255):
        super(MscLovaszSoftmax, self).__init__()
        self.lovza = LovaszSoftmax(ignore_index=ignore_index)
    ...
    def forward(self, input, target):
        ...
        for item in input:
            h, w = item.size(2), item.size(3)
            item_target = F.interpolate(target.unsqueeze(1).float(), size=(h, w), mode='nearest')
            loss += self.lovza(item, item_target)
        return loss / len(input)


class MscBCELoss(nn.Module):
    def __init__(self, n_classes, ignore_index=255):
        super(MscBCELoss, self).__init__()
...
def focal_tversky_loss_func(y_pred, y_true, smooth=1, gamma=0.75):
    ...
    alpha = 0.7
    pt_1 = (true_pos + smooth) / (true_pos + alpha * false_neg + (
...
# class Seg_Yolo_focal_loss(nn.Module):
#     def __init__(self, ignore_index=255):
#         super(Seg_Yolo_focal_loss, self).__init__()
#         self.ignore_index = ignore_index
#
#     def forward(self, predict, target):
#         b, c, h, w = predict.shape
#         predict = predict.permute(0, 2, 3, 1).contiguous().view(-1, c)
#         target = target.view(-1)
#         mask = (target != self.ignore_index)  #### source
#         # mask = (target < 16)
#         predict = predict[mask]
#         target = target[mask]
#         target_one_hot = F.one_hot(target, 16).float()
#         ## loss replace
#         # predict = torch.sigmoid(predict)
#         # loss = F.binary_cross_entropy(input=predict, target=target_one_hot, reduction='mean')
#         ## loss replace
#         loss = seg_focal_loss(predict, target_one_hot)
#         return loss
...
class OhemCELossV2(nn.Module):
    ...
    def forward(self, logits, labels):
        ...
        loss_summary = []
        for i in range(16):
            val_sum, val_num = torch.sum(loss[label_flatten == i]), (label_flatten == i).sum()
            loss_summary.append([val_sum, val_num])
        ...
        if loss[self.n_min] > self.thresh:
            ...
            loss = loss[:self.n_min]
        return torch.mean(loss), loss_summary
