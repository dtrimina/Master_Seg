...
from losses.loss import LovaszSoftmax, dice_loss_func, ProbOhemCrossEntropy2d
...
class Loss(nn.Module):
    ...
    def __init__(self):
        ...
        self.wbce = nn.CrossEntropyLoss(weight=class_weight).to(device)
        self.bcelogits = nn.BCEWithLogitsLoss().to(device)
        self.lovza = LovaszSoftmax()
        self.dice = dice_loss_func
        self.ohem = ProbOhemCrossEntropy2d()
        self.wohem = ProbOhemCrossEntropy2d(weight=class_weight)
    ...
    def forward(self, pred_dict, gt_dict):
        ...
        #     'label_boundary': torch.from_numpy(
        #         np.asarray(segimg.label_boundary_map(kernel_size=self.boundary_kernel_size), dtype=np.int64)).long(),
        ...
        # saliency loss
        loss_saliency_dice = self.dice(torch.sigmoid(pred_dict['pred_saliency']), gt_dict['label_saliency'])
        loss_saliency_bce = self.bcelogits(pred_dict['pred_saliency'], gt_dict['label_saliency'])

        #

        loss_dict = {
            'loss': loss_saliency_bce
        }
...
def run(cfg_path):
    ...
    # 定义模型
    ####################
    from models.dddmodel import ModelDual
    model = ModelDual(n_classes=N_CLASSES)
    model.to(device)
    # 加载预训练模型
    ...
    for ep in range(cfg['epochs']):
        ...
        for i, sample in tqdm(enumerate(train_loader), total=len(train_loader)):
            ...
            with autocast():
                predict_dict = model(image, depth)
                loss_dict = train_criterion(predict_dict, sample)
                loss = loss_dict['loss']
